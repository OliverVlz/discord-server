...
const setupApiRoutes = (discordClient) => {
    app.post('/generate-invite', async (req, res) => {
        ...
        try {
            ...
            const existingInviteResult = await (0, database_1.query)(`SELECT invite_code, created_at FROM invites WHERE email = $1 AND status = 'PENDING'`, [email]);
            ...
            if (existingInvite) {
                ...
                if (new Date() < expiresAt) {
                    ...
                    await (0, database_1.query)(`UPDATE invites SET status = 'EXPIRED' WHERE invite_code = $1`, [existingInvite.invite_code]);
            ...
            if (!existingInvite) {
                ...
                const channelId = process.env.DEFAULT_CHANNEL_ID; // Ahora lee del .env
                ...
                const invitationalChannel = channel;
                ...
                await (0, database_1.query)('INSERT INTO invites (invite_code, role_id, email, status, created_at, expires_at) VALUES ($1, $2, $3, $4, $5, $6)', [invite.code, roleId, email, 'PENDING', createdAt, expiresAt]);
